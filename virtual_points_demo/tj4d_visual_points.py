...
def process_one_frame(instances,points,extrinsic,intrinsic):
    ...
    #points filter
    point_mask = (points[:, 2] > -5) & (points[:, 0] < 70) & (points[:, 2] < 5)
    points = points[point_mask]


    points_in_img, mask = get_fov_mask(points[:, :3], extrinsic, intrinsic, h=960, w=1280)
...
if __name__ == "__main__":
    ...
    path_base = 'data/TJ4DRadSet'
    txt_path = os.path.join(path_base, 'TJ4DRadSet_4DRadar', 'ImageSets', 'trainval.txt')
    ...
    for prefix in prefixes:
        ...
        radar_path = os.path.join(path_base, 'TJ4DRadSet_4DRadar/training/velodyne', prefix + '.bin')
        calib_path = os.path.join(path_base, 'TJ4DRadSet_4DRadar/training/calib', prefix + '.txt')
        img_path = os.path.join(path_base, 'TJ4DRadSet_Non_Public/training/image_2',prefix + '.png')
        ...
        use_dim = [0, 1, 2, 3, 5]
        radar_points = radar_points[:, use_dim]
