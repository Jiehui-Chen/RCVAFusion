...
class CreateProjectionPoints:
    def __init__(self, coord_type: str, filter_min, filter_max):
        # filter_min_max
    ...
    def __call__(self, results):
        ...
        radar_point = results['points'].tensor.numpy()
        radar_point_feature = radar_point[:, 3:]
        radar_point = radar_point[:, :3]
        ...
        img_pts = img_pts[valid_mask][:,[0,2,1]]
        radar_point_feature = radar_point_feature[valid_mask]
        projection_points = np.concatenate([img_pts[:, :3], radar_point_feature], axis=1)
        attribute_dims = None

        points_class = get_points_type(self.coord_type)
        projection_points = points_class(
            projection_points, points_dim=projection_points.shape[-1], attribute_dims=attribute_dims)

        results['projection_points'] = projection_points

        return results

@TRANSFORMS.register_module()
class CreateProjectionCoorPoints:
    def __init__(self, coord_type: str, filter_min, filter_max):
        # filter_min_max
        self.filter_min = filter_min
        self.filter_max = filter_max
...
class CreateDepthFromLiDAR:
    ...
    def __call__(self, results):
        ...
        if self.dataset == 'kitti':
            ...
            lidar_filename = os.path.join(self.data_root, train_or_test, "velodyne",
                                          results['lidar_points']['lidar_path'])
        ...
        if self.dataset == 'VoD':
            ...
            training_or_testing = results['img_path'].split('/')[4]
            index_num = results['img_path'].split('/')[-1].split('.')[0]
            ...
            rect = np.array([float(info) for info in lines[4].split(' ')[1:10]]).reshape([3, 3]);
            rect_4x4 = np.zeros([4, 4], dtype=rect.dtype);
            rect_4x4[3, 3] = 1.;
            rect_4x4[:3, :3] = rect;
            rect = rect_4x4
            Trv2c = np.array([float(info) for info in lines[5].split(' ')[1:13]]).reshape([3, 4]);
            Trv2c = self._extend_matrix(Trv2c)
        ...
        if self.dataset == 'TJ4D':
            ...
            training_or_testing = results['img_path'].split('/')[3]
            index_num = results['img_path'].split('/')[-1].split('.')[0]
            lidar_calib_path = os.path.join(lidar_root, 'TJ4DRadSet_LiDAR',training_or_testing, 'calib', index_num + '.txt')
            with open(lidar_calib_path, 'r') as f:
                lines = f.readlines()
            P2 = np.array([float(info) for info in lines[2].split(' ')[1:13]]).reshape([3, 4]);
            P2 = self._extend_matrix(P2)
