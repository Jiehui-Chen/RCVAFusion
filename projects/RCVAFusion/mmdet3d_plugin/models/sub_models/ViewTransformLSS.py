...
class ViewTransformerLSS_VRPDL(nn.Module):
    def __init__(self, grid_config=None, data_config=None, downsample=8, num_in_height=8, occupancy_height=112):
        super().__init__()
        ...
        self.img_depth = nn.Sequential(
            nn.Conv2d(in_channels, self.D, kernel_size=3, stride=1, padding=1))
        self.radar_occupancy = nn.Sequential(
            nn.Conv2d(192, occupancy_height, kernel_size=3, stride=1, padding=1))
    ...
    def create_frustum(self):
        ...
        if self.training:
            ogfH, ogfW = self.data_config['final_dim']
        else:
            ogfH, ogfW = self.data_config['final_dim_test']
    ...
    def get_cam_feats(self, x, projection_bev):
        B, N, C, fH, fW = x.shape
        x = x.view(B * N, C, fH, fW)
        radar_occupancy = self.sigmoid(self.radar_occupancy(projection_bev).permute(0, 2, 1, 3).contiguous())
        img_depth = self.img_depth(x).softmax(dim=1)

        x_1 = radar_occupancy.unsqueeze(1) * x.unsqueeze(2)
        x_2 = img_depth.unsqueeze(1) * x.unsqueeze(2)


        x_1 = x_1.view(B, N, self.C, self.D, fH, fW)
        x_1 = x_1.permute(0, 1, 3, 4, 5, 2)
        x_2 = x_2.view(B, N, self.C, self.D, fH, fW)
        x_2 = x_2.permute(0, 1, 3, 4, 5, 2)
        return x_1, x_2
    ...
    def forward(self, feat, radar, cam_params):
        ...
        img_association_4d, img_4d = self.get_cam_feats(feat, radar)

        # Splat
        geom = self.get_geometry(rots, trans, intrins, post_rots, post_trans, bda)
        img_association_feats = self.voxel_pooling(geom, img_association_4d)
        img_bev_feat = self.voxel_pooling(geom, img_4d)


        return img_association_feats, img_bev_feat  # [b, c, x, y, z]
