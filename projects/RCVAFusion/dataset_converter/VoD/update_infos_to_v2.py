...
def update_VOD_infos(pkl_path, out_dir):
    ...
    METAINFO = {
        'classes': ('Car', 'Pedestrian', 'Cyclist', 'rider','truck','bicycle','motor','moped_scooter','ride_other','bicycle_rack','human_depiction','vehicle_other','ride_uncertain','DontCare'),
    }
    ...
    for ori_info_dict in mmengine.track_iter_progress(data_list):
        ...
        temp_data_info['images']['CAM0']['cam2img'] = ori_info_dict['calib']['calib_lidar'][
            'P0'].tolist()
        temp_data_info['images']['CAM1']['cam2img'] = ori_info_dict['calib']['calib_lidar'][
            'P1'].tolist()
        temp_data_info['images']['CAM2']['cam2img'] = ori_info_dict['calib']['calib_lidar'][
            'P2'].tolist()
        temp_data_info['images']['CAM3']['cam2img'] = ori_info_dict['calib']['calib_lidar'][
            'P3'].tolist()
        ...
        temp_data_info['lidar_points']['num_pts_feats'] = ori_info_dict[
            'lidar_point_cloud']['num_features']
        temp_data_info['radar_points']['num_pts_feats'] = ori_info_dict[
            'radar_point_cloud']['num_features']

        temp_data_info['lidar_points']['lidar_path'] = Path(
            ori_info_dict['lidar_point_cloud']['velodyne_path']).name
        temp_data_info['radar_points']['radar_path'] = Path(
            ori_info_dict['radar_point_cloud']['velodyne_path']).name

        rect_lidar = ori_info_dict['calib']['calib_lidar']['R0_rect'].astype(np.float32)
        Trv2c_lidar = ori_info_dict['calib']['calib_lidar']['Tr_velo_to_cam'].astype(np.float32)
        lidar2cam = rect_lidar @ Trv2c_lidar
        temp_data_info['lidar_points']['lidar2cam'] = lidar2cam.tolist()
        temp_data_info['lidar_points']['lidar2img'] = (
            ori_info_dict['calib']['calib_lidar']['P2'] @ lidar2cam).tolist()

        rect_radar=ori_info_dict['calib']['calib_radar']['R0_rect'].astype(np.float32)
        Trv2c_radar=ori_info_dict['calib']['calib_radar']['Tr_velo_to_cam']
        radar2cam = rect_radar @ Trv2c_radar
        temp_data_info['radar_points']['radar2cam'] = radar2cam.tolist()
        temp_data_info['radar_points']['radar2img'] = (
                ori_info_dict['calib']['calib_radar']['P2'] @ radar2cam).tolist()
        ...
        temp_data_info['lidar_points']['Tr_velo_to_cam'] = Trv2c_lidar.tolist()
        temp_data_info['radar_points']['Tr_velo_to_cam'] = Trv2c_radar.tolist()

        # for potential usage
        temp_data_info['images']['R0_rect'] = ori_info_dict['calib']['calib_lidar'][
            'R0_rect'].astype(np.float32).tolist()


        cam2img = ori_info_dict['calib']['calib_lidar']['P2']
        ...
        if anns is not None:
            ...
            for instance_id in range(num_instances):
                ...
                #     instance_id].tolist()
                empty_instance['num_lidar_pts'] = anns['lidar_num_points_in_gt'][
                    instance_id].tolist()
                empty_instance['num_radar_pts']=anns['radar_num_points_in_gt'][instance_id].tolist()
    ...
    dataset_root = os.path.dirname(pkl_path)
    out_path = osp.join(dataset_root, out_dir)
